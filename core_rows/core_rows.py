"""
Програма для побудови покриття методом ядерних рядків.
    ...
    for main_row_index in range(m_c_rows):
        for secondary_row_index in range(m_c_rows):
            ...
            if main_row_index != secondary_row_index:
                ...
                if break_out_flag:
                    continue
                if count_ones(matrix_copy[main_row_index]) <= count_ones(matrix_copy[secondary_row_index]) and main_row_index not in absorbed_rows:
